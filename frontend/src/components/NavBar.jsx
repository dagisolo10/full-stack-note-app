...
import { Hamburger, HamburgerIcon, LogIn, LogOut, Menu, UserPlusIcon, X } from "lucide-react";
...
const NavBar = () => {
    ...
    return (
        <div className="bg-transparent mb-4 w-full px-6 py-3 flex gap-4 justify-end items-center">
            <div className={`fixed top-0 ${menu ? "right-0" : "-right-100"} duration-500 bg-gray-800 h-screen min-w-3/5 z-50 sm:hidden flex flex-col gap-6 items-start p-6`}>
                <button>
                    <X onClick={() => setMenu((p) => (p = !p))} className="sm:hidden" />
                </button>
                {location !== "/" && (
                    <Link
                        to="/"
                ...
                {location === "/" && (
                    <Link to="/signup" className="duration-500 text-primary-content hover:text-gray-700">
                        <span>Signup</span>
                    </Link>
                )}
                {location === "/" && (
                    <Link to="/login" className="duration-500 text-primary-content hover:text-gray-700">
                        <span>Login</span>
                    </Link>
                )}
            ...
            <button>
                <Menu onClick={() => setMenu((p) => (p = !p))} className="sm:hidden" />
            </button>
            {user && location !== "/" && location !== "/admin-dashboard" && (
                <Link to="/admin-dashbaord">
